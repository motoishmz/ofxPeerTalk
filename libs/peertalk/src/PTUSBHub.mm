...
static usbmux_packet_t *usbmux_packet_alloc(CFIndex payloadSize) {
	...
	usbmux_packet_t *upacket = (usbmux_packet_t *)CFAllocatorAllocate(kCFAllocatorDefault, upacketLength, 0);
...
- (void)scheduleReadPacketWithCallback:(void(^)(NSError*, NSDictionary*, uint32_t))callback {
	...
	dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
		...
		// !!!:
		// PT_PRECISE_LIFETIME_UNUSED
		dispatch_data_t map_data = dispatch_data_create_map(data, (const void **)&buffer, &buffer_size); // objc_precise_lifetime guarantees 'map_data' isn't released before memcpy has a chance to do its thing
		...
		dispatch_io_read(channel_, offset, upacket->size - offset, queue_, ^(bool done, dispatch_data_t data, int error) {
			...
			memcpy(((char *)(upacket)+offset), (const void *)buffer, buffer_size);
#if PT_DISPATCH_RETAIN_RELEASE
			dispatch_release(map_data);
#endif
