...
+ (PTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
	//!!!:FIXME
  static const char currentQueueFrameProtocolKey = '\0';
...
- (dispatch_data_t)createDispatchDataWithFrameOfType:(uint32_t)type frameTag:(uint32_t)frameTag payload:(dispatch_data_t)payload {
  PTFrame *frame = (PTFrame *)CFAllocatorAllocate(kCFAllocatorDefault, sizeof(PTFrame), 0);
...
#endif
	//!!!:FIXME
  static const bool kDispatchDataRefKey = false;
