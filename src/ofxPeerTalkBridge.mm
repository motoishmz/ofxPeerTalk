...
#import "ofxPeerTalkProtocol.h"
#import "PTUSBHub.h"

@interface ofxPeerTalkBridge () {
...
static const NSTimeInterval ofxPeerTalkReconnectDelay = 1.0;

@implementation ofxPeerTalkBridge
...
- (void)didDisconnectFromDevice:(NSNumber*)deviceID {
	NSLog(@"Disconnected from device: %@", deviceID);
...
- (void)connectToLocalIPv4Port {
	...
	channel.userInfo = [NSString stringWithFormat:@"127.0.0.1:%d", ofxPeerTalkProtocolIPv4PortNumber];
	[channel connectToPort:ofxPeerTalkProtocolIPv4PortNumber IPv4Address:INADDR_LOOPBACK callback:^(NSError *error, PTAddress *address) {
		if (error) {
			if (error.domain == NSPOSIXErrorDomain && (error.code == ECONNREFUSED || error.code == ETIMEDOUT)) {
				// this is an expected state
			} else {
				NSLog(@"Failed to connect to 127.0.0.1:%d: %@", ofxPeerTalkProtocolIPv4PortNumber, error);
			}
		} else {
			[self disconnectFromCurrentChannel];
		...
		[self performSelector:@selector(enqueueConnectToLocalIPv4Port) withObject:nil afterDelay:ofxPeerTalkReconnectDelay];
	}];
}
...
- (void)connectToUSBDevice {
	...
	[channel connectToPort:ofxPeerTalkProtocolIPv4PortNumber overUSBHub:PTUSBHub.sharedHub deviceID:connectingToDeviceID_ callback:^(NSError *error) {
		if (error) {
			...
			if (channel.userInfo == connectingToDeviceID_) {
				[self performSelector:@selector(enqueueConnectToUSBDevice) withObject:nil afterDelay:ofxPeerTalkReconnectDelay];
			}
		} else {
